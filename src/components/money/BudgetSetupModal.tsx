...
export const BudgetSetupModal: React.FC<BudgetSetupModalProps> = ({
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const updated: BudgetProfile = {
      ...
      categoryCaps: {
        Food: Math.max(0, Number(foodMonthly)),
        Travel: Math.max(0, Number(travelMonthly)),
        Health: Math.max(0, Number(healthMonthly)),
        Housing: Math.max(0, Number(housingMonthly)),
        LoanClearance: Math.max(0, Number(loanClearanceMonthly)),
        FamilyContribution: Math.max(0, Number(familyContributionMonthly)),
        Learning: Math.max(0, Number(learningMonthly)),
        Entertainment: freeCapital,
      },
